...
struct MemReq {
    ...
    enum Flag {
        ...
        SW_SPECULATIVE      = (1<<7), //Access generated by software prefetch instruction. Whenever SW_SPECULATIVE is set, SPECULATIVE needs to be set as well
    };
    uint32_t flags;
...
static inline bool isDemandLoad(const MemReq *req) {
    if (req && !(req->flags & MemReq::SPECULATIVE) &&
        !(req->flags & MemReq::SW_SPECULATIVE) &&
        !req->prefetch && req->type == GETS && req->pc != 0L) {
        return true;
    }
    return false;
}
...
static inline bool isDemandStore(const MemReq *req) {
    if (req && !(req->flags & MemReq::SPECULATIVE)
        && !(req->flags & MemReq::SW_SPECULATIVE)
        && !req->prefetch && req->type == GETX && req->pc != 0L) {
        return true;
    }
    return false;
}
