...
static inline bool isFlagsReg(uint32_t reg) {
    return (reg == XED_REG_EFLAGS || reg == XED_REG_FLAGS || reg == XED_REG_MXCSR);
}
...
static std::string regsToString(uint32_t* regs, uint32_t numRegs) {
    ...
    if (numRegs) {
        ...
        std::string tmp(xed_reg_enum_t2str((const xed_reg_enum_t)regs[numRegs - 1]));
        str += tmp + ")";
    }
    return str;
}
...
void Decoder::reportUnhandledCase(Instr& instr, const char* desc) {
    warn("Unhandled case: %s | %s | loads=%d stores=%d inRegs=%d %s outRegs=%d %s", desc, xed_iclass_enum_t2str( (xed_iclass_enum_t) INS_Opcode(instr.ins)),
...
void Decoder::emitBasicOp(Instr& instr, DynUopVec& uops, uint32_t lat, uint8_t ports, uint8_t extraSlots, bool reportUnhandled) {
    ...
    // Tolerate instructions with more than two register operands by ignoring the additional operands.
...
bool Decoder::decodeInstr(const INS ins, uint64_t pc, DynUopVec& uops) {
    ...
    info("category %d opco %d last cat %s opcode: %s", category, opcode, xed_category_enum_t2str(category), xed_iclass_enum_t2str(opcode) );
#endif
        ...
        case XC(STRINGOP):
            switch (opcode) {
                ...
                case XO(CMPSQ):
                    ...
                    emitExecUop(REG_LOAD_TEMP, REG_LOAD_TEMP+1, XED_REG_RFLAGS, 0, uops, 1, PORT_5);
                    emitExecUop(XED_REG_RSI, 0, XED_REG_RSI, 0, uops, 1, PORTS_015);
                    emitExecUop(XED_REG_RDI, 0, XED_REG_RDI, 0, uops, 1, PORTS_015);
                    break;
...
bool Decoder::decodeCustomInstr(CustomOp cop, DynUopVec& uops) {
    ...
    switch (cop) {
        case CustomOp::PREFETCH_CODE: {
            ...
            // prefetching (it acts as a NOP in the pipeline). Add the dependencies to support regsiter-based
            // addressing similar to XC(PREFETCH) instructions.
            DynUop uop;
            uop.clear();
            uop.rs[0] = 0;
            uop.rs[1] = 0;
...
bool Decoder::decodeFusedInstrs(const INS ins, uint64_t pc, const INS next, DynUopVec& uops) {
    ...
    if (oooDecoding) {
        ...
        while (trace_cur != bbl->end()) {
            ...
            if (unlikely(trace_cur->custom_op != CustomOp::NONE)) {
                ...
                instrBytes.push_back(0);  // TODO(granta): Decide whether or not to fake instruction sizes
                ...
                if (trace_next != bbl->end() && Decoder::canFuse(trace_cur->ins, trace_next->ins)) {
                    inaccurate = Decoder::decodeFusedInstrs(trace_cur->ins, trace_cur->pc, trace_next->ins, uopVec);
                    instrAddr.push_back(trace_cur->pc);
                    ...
                    inaccurate = Decoder::decodeInstr(trace_cur->ins, trace_cur->pc, uopVec);
                    instrAddr.push_back(trace_cur->pc);
                    instrBytes.push_back(INS_Size(trace_cur->ins));
                    instrUops.push_back(uopVec.size() - prevUops);
                    instrDesc.push_back(*trace_cur);
        ...
        for (uint32_t i = 0; i < instrs; i++) {
            ...
            bool simple = ((instrUops[i] - instrFusable[i]) == 1) && (instrBytes[i] < 8);
